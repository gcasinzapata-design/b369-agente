...
export async function POST(req: NextRequest) {
  try {
    ...
    const resultsRaw = [];
    if (!key || !cx) return NextResponse.json({ results: [], alternatives: [], error: 'Faltan GOOGLE_API_KEY o GOOGLE_CSE_ID' });
    for (const query of q) {
      try {
        const url = `https://www.googleapis.com/customsearch/v1?key=${key}&cx=${cx}&q=${encodeURIComponent(query)}&num=8`;
    ...
    // enrich: try to fetch page to get phone/name (best effort)
    const enriched = [];
    for (const item of norm.slice(0,40)) {
      try {
        const { data: html } = await axios.get(item.url, { timeout, headers: { 'User-Agent': 'Mozilla/5.0' } });
        const phone = html.match(/(\+51\s?)?9\d{8}/)?.[0];
        enriched.push({ ...item, contactPhone: phone, lat: centroid?.lat, lng: centroid?.lng });
      } catch { enriched.push({ ...item, lat: centroid?.lat, lng: centroid?.lng }); }
    }
    // simple scoring based on presence
    const results = enriched.slice(0,10);
    const alternatives = enriched.slice(10,20).map(r=>({ ...r, summary: (r.summary||'') + ' (alternativa)' }));
